...
function makeTwo() {
	fetch('https://emojihub.yurace.pro/api/random/group/face-positive')
		...
		.then(data => {
			...
			resultParagraph.innerHTML = data.htmlCode;
		})
...
function makeFive() {
	fetch('https://www.boredapi.com/api/activity')
		.then(response => response.json())
		.then(data => console.log(`Количество участников: ${data.participants}`))
		.catch(error => console.log(error));
}
...
function makeTen() {
	fetch('https://dog.ceo/api/breeds/image/random')
		...
		.then(data => {
			...
			result10.insertAdjacentElement('afterend', imageElement);
		})
		.catch(error => console.log(error));
}
...
function makeEleven() {
	fetch('https://api.ipify.org?format=json')
		...
		.then((data) => {
			...
			result11.innerHTML = JSON.stringify(data.ip);
		})
		.catch((error) => {
			console.error('Ошибка:', error);
		});
...
function makeTwelve() {
	const inputValue = document.getElementById('ipAddress').value;
	console.log(inputValue);
	fetch(`http://ip-api.com/json/${inputValue}`)
		.then(response => response.json())
		.then(data => {
			const geo = `latitude: ${data.lat}, longitude: ${data.lon}`;
			document.getElementById("result12").textContent = geo;
		})
		.catch(error => console.log(error));
}
...
function makeFourteen() {
	fetch('https://official-joke-api.appspot.com/random_joke')
		.then((response) => response.json())
		.then((data) => {
			document.getElementById('result14').innerHTML = `setup: ${data.setup} <br> punchline: ${data.punchline}`;
		})
...
function makeSixteen() {
	fetch('https://jsonplaceholder.typicode.com/posts/1', {
		...
		body: JSON.stringify({ title: 'Новый заголовок', body: 'Новый текст поста' })
	})
		.then(response => response.json())
		.then(data => console.log(data))
		.catch(error => console.log(error));
...
function makePromiseAllOne() {
	const promises = [fetch('https://jsonplaceholder.typicode.com/posts/1').then(response => response.json()), fetch('https://jsonplaceholder.typicode.com/comments/1').then(response => response.json())];
